...
/**
 * Created by lucky on 2018/12/19 0019
 * Here be dragon
 * 前方高能
 */
...
public class CustToast extends Toast {
     ...
     * Toast单例
     */
    private static CustToast toast;
    ...
    private static void initToast(Context context, CharSequence text) {
        try {
            ...
            // 由layout文件创建一个View对象
            View layout = inflater.inflate(R.layout.layout_cust_toast, null);
    ...
    private static void showToast(Context context, CharSequence text, int time, int imgType) {
        ...
        if (imgType == TYPE_HIDE) {
            ...
            if (imgType == TYPE_TRUE) {
                toast_img.setBackgroundResource(R.drawable.icon_right);
            } else {
                toast_img.setBackgroundResource(R.drawable.icon_wrong);
            }
